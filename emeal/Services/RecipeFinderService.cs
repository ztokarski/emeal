...
namespace emeal.Models.Utils
{
    public class RecipeFinderService : IRecipeFinder
    {
        ...
        public List<int> FindRelevantRecipes(List<Recipe> allRecipes, List<int> queryArr)
        {
            ...
            int maxProductNumberDifference = 5;
            ...
            foreach (var recipe in allRecipes)
            {
                ...
                int queryLen = queryArr.Count;
                int recipeLen = productsIds.Count;

                if(Math.Abs(queryLen - recipeLen)<= maxProductNumberDifference)
                {
                    if (exceptQueryRecipe == queryLen)
                        continue;
            ...
            foreach (var tuple in recipeCoefficient)
            {
